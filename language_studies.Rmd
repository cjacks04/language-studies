...
newcomer_info_commenters <- newcomer_info_commenters[,c(1:39,41,42,40)]
...
retention_commentersup.glm <- glm(Class ~ newcomer_class+newcomer_comment+newcomer_sessions+newcomer_first_comment_seq+dslope+sslope+domain_cosine+science_cosine+questions+links+words+since_start+newcomer_first_comment_ses+start_count,data = traincommenterup,family="binomial")
...
# Science cosine and change
science.lmer <- lmer(science_cosine ~ week_class +week_comment + questions + links + words + last_session + week + (1|user_id) , data = week_model,REML=FALSE)
