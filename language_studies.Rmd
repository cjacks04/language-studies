...
```{r}
all_comments <- read_csv("/Volumes/cbjackson2/Zooniverse Datasets/gravityspy/gravity-spy-comments_2022-07-15.csv") # move to Zoonvierse Datasets folder

malletwords <- scan("/Volumes/cbjackson2/language models/mallet.txt", character(), quote = "")

all_comments$created_at <- as.POSIXct(all_comments$comment_created_at , format="%Y-%m-%d %H:%M:%S")
all_comments <- all_comments[which(all_comments$created_at <= "2022-03-14 15:39:44"),]


# Data transformations
all_comments$comment_body <-  gsub("http[[:alnum:][:punct:]]*", "", all_comments$comment_body)# remove URLs
all_comments$comment_body <-  gsub("http[^[:space:]]*", "", all_comments$comment_body) # remove non-ASCII characters
all_comments$comment_body <-  removeNumbers(all_comments$comment_body) # remove non-ASCII characters
all_comments$filtered_wordsnew <- tolower(gsub('[[:punct:]]', ' ', all_comments$comment_body))
all_comments$filtered_wordsnew <- removeWords(all_comments$filtered_wordsnew, c(stopwords("english"),malletwords))

#rename old comment column
all_comments$filtered_words <- NULL
all_comments$filtered_words <- all_comments$filtered_wordsnew
all_comments$filtered_wordsnew <- NULL

# parse comment and hashtag datasets to unigrams
unigram_comments <- all_comments %>% unnest_tokens(unigram, 
                                               filtered_words, token = "ngrams", n = 1)



all_comments_summary <- unigram_comments %>% 
  #group_by(week=floor_date(comment_created_at, "week")) %>% 
  summarize(
    words=length(unigram),
    words_unique=length(unique(unigram)),
    threads = length(unique(discussion_id))
    )

all_comments_week_summary <- unigram_comments %>% 
  group_by(week=floor_date(comment_created_at, "week")) %>% 
  summarize(
    words=length(unigram),
    words_unique=length(unique(unigram)),
    commenters=length(unique(comment_user_login)),
    threads=length(unique(discussion_id))
    )

# get people joining discussion spaces 
user_commenting <- all_comments %>% 
  group_by(comment_user_login) %>% 
  summarize(
    week=min(floor_date(comment_created_at, "week")),
    )

user_commenting_week <- user_commenting  %>%  
  group_by(week) %>% 
  summarize(
    newcomers=length(comment_user_login))

all_comments_week_summary <- merge(all_comments_week_summary,user_commenting_week,by="week",all.x = TRUE)
remove(user_commenting,user_commenting_week)

# get new threads created 
threads_commenting <- all_comments %>% 
...
threads_commenting_week <- threads_commenting  %>%  
  group_by(week) %>% 
  summarize(
    newthreads=length(discussion_id))

all_comments_week_summary <- merge(all_comments_week_summary,threads_commenting_week,by="week",all.x = TRUE)
remove(threads_commenting,threads_commenting_week)

# get new words created 
words_commenting <- unigram_comments %>% 
  group_by(unigram) %>% 
  summarize(
    week=min(floor_date(comment_created_at, "week")),
    )
words_commenting_week <- words_commenting  %>%  
  group_by(week) %>% 
  summarize(
    newwords=length(unigram))

all_comments_week_summary <- merge(all_comments_week_summary,words_commenting_week,by="week",all.x = TRUE)
remove(words_commenting,words_commenting_week)

all_comments_week_summary[is.na(all_comments_week_summary)] <- 0

all_comments_week_summary <-  all_comments_week_summary %>%
  mutate(newcomer_growth = cumsum(newcomers),
         newcomer_pct_change = (newcomer_growth/lag(newcomer_growth) - 1) * 100,
         thread_growth = cumsum(newthreads),
         thread_pct_change = (thread_growth/lag(thread_growth) - 1) * 100,
         word_growth = cumsum(newwords),
         word_pct_change = (word_growth/lag(word_growth) - 1) * 100) 


# Visualizations showing community growth analysis
if(!require(mice)){install.packages("mice")}
if(!require(Kendall)){install.packages("Kendall")}
if(!require(trend)){install.packages("trend")}
#install.packages("funtimes")
library(funtimes) #https://cran.r-project.org/web/packages/funtimes/funtimes.pdf 
# install.packages(path_to_file, repos = NULL, type="source")
 
# Examples https://cran.r-project.org/web/packages/funtimes/vignettes/trendtests.html 
apply(all_comments_week_summary[,c(6:8)], 2, function(x) notrend_test(x)$p.value)
apply(all_comments_week_summary[,c(6:8)], 2, function(x) notrend_test(x, test = "MK")$p.value)
apply(all_comments_week_summary[,c(6:8)], 2, function(x) notrend_test(x, test = "WAVK", 
                                     factor.length = "adaptive.selection")$p.value)

MannKendall(all_comments_week_summary$newcomers)
MannKendall(all_comments_week_summary$newthreads)
MannKendall(all_comments_week_summary$newwords)

## Visualizations for community growth
all_comments_week_summary$project_week <- as.numeric(1:nrow(all_comments_week_summary))
all_comments_week_summary$week <- as.Date(all_comments_week_summary$week)

# visualizations


newcomer_growth <-  ggplot(subset(all_comments_week_summary, project_week > 30) ,aes(y=newcomer_pct_change, x=week)) + 
  geom_line(stat="identity",size=.7) + labs(x="",y="") + ggtitle("Newcomers") + geom_smooth() +
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1),
        legend.position="bottom", legend.title = element_blank()) + 
geom_vline(aes(xintercept=as.Date("2016-10-12")),linetype="dashed", color = "grey22",size=.7) 

thread_growth <-  ggplot(subset(all_comments_week_summary, project_week > 30),aes(y=thread_pct_change, x=week)) + 
  geom_line(stat="identity",size=.7) + labs(x="Month",y="") + ggtitle("New threads") + geom_smooth() +
  #scale_x_date(date_breaks = "2 weeks", date_labels = "%W") +
  scale_x_date(date_labels="%b %y",date_breaks  ="4 month") +  
  scale_y_continuous(labels = function(x) paste0(x, "%")) +
  theme_minimal() + 
...
word_growth <- ggplot(subset(all_comments_week_summary, project_week > 30),aes(y=word_pct_change, x=week)) + 
  geom_line(stat="identity",size=.7) + labs(x="",y="Percentage increase") + ggtitle("New words") + geom_smooth() +
  #scale_x_date(date_breaks = "2 weeks", date_labels = "%W") +
  scale_x_date(date_labels="%b %y",date_breaks  ="4 month") +  
  scale_y_continuous(labels = function(x) paste0(x, "%")) +
  theme_minimal() + 
...
pdf("/Users/jackson/Library/CloudStorage/Box-Box/_research/language[collab]/www22-figures/word_growth.pdf", width=5, height=5)
word_growth
dev.off() 

pdf("/Users/jackson/Library/CloudStorage/Box-Box/_research/language[collab]/www22-figures/thread_growth.pdf", width=5, height=5)
thread_growth
dev.off() 

pdf("/Users/jackson/Library/CloudStorage/Box-Box/_research/language[collab]/www22-figures/newcomer_growth.pdf", width=5, height=5)
newcomer_growth
dev.off() 
