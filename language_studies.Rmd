...
pdf("/Users/coreyjackson/Library/CloudStorage/Box-Box/_research/language/group2025/figures/figure 4_agg.pdf", width=30, height=15)
feature_growth
dev.off()

png("/Users/coreyjackson/Library/CloudStorage/Box-Box/_research/language/group2025/figures/figure 4_agg.png", width=30, height=15)
feature_growth
dev.off()

tiff("/Users/coreyjackson/Library/CloudStorage/Box-Box/_research/language/group2025/figures/figure 4_agg.tiff", units="cm", width = 30, height = 15, res=300)
feature_growth
dev.off()
...
pdf("/Users/coreyjackson/Library/CloudStorage/Box-Box/_research/language/group2025/figures/figure 4_count.pdf ", width=30, height=15)
feature_count
dev.off()

png("/Users/coreyjackson/Library/CloudStorage/Box-Box/_research/language/group2025/figures/figure 4_count.png", units="cm", width = 30, height = 15, res=300)
feature_count
dev.off()

tiff("/Users/coreyjackson/Library/CloudStorage/Box-Box/_research/language/group2025/figures/figure 4_count.tiff", units="cm", width = 30, height = 15, res=300)
feature_count
dev.off()
...
growth_pettit_count <- eco_count_plot.m %>% 
  group_by(variable) %>% 
...
growth_sens_count <- eco_count_plot.m %>% 
   group_by(variable) %>% 
...
not_eligible <- newcomers_data$user_id[which(newcomers_data$join_date > max(all_comments$comment_created_at) - months(1))]
...
newcomer_comments$include <- ifelse(newcomer_comments$comment_created_at <= add_with_rollback(newcomer_comments$join_date, months(1), roll_to_first = TRUE),1,0)
...
### Add thread info
thread_first_user <- merge(thread_first_user,newcomers_data[,c("user_id","join_date")],by="user_id",all.x = TRUE)

thread_first_user$include <- ifelse(thread_first_user$week <= add_with_rollback(thread_first_user$join_date, months(2), roll_to_first = TRUE),1,0)
thread_first_user <- thread_first_user[which(thread_first_user$include==1),]
thread_first_user <- thread_first_user[which(thread_first_user$user_id %in% population),] # only eligible people

thread_create <- thread_first_user  %>% 
  group_by(user_id) %>%
  summarise(new_threads=sum(threads_started))

newcomers_data <- merge(newcomers_data,thread_create,by="user_id",all.x = TRUE)
newcomers_data$new_threads[is.na(newcomers_data$new_threads)] <- 0  #fill NA with 0 

newcomers_week_data <- merge(newcomers_week_data,thread_first_user[,c("user_id","week","threads_started")],by=c("user_id","week"),all.x = TRUE)
newcomers_week_data$threads_started[is.na(newcomers_week_data$threads_started)] <- 0  #fill NA with 0 
```
...
user_count_plot.summary$variable <- factor(user_count_plot.summary$variable, 
                                            levels=c("Words(Field)", "Cosine(Field)", "Change(Field)",
                                                     ...
                                                     "Comments","Threads","Tokens","Tags","Questions","URLs","@s"))

## HIST
feature_commentB <- ggplot(subset(user_count_plot.summary, ! variable %in% c("Comments","Threads","Questions","URLs","Tokens","Tags","@s",NA)), aes(x=user_week, y=value)) + 
...
user_count_plot.m$variable <-  recode(user_count_plot.m$variable, comment_no = "Comments",threads_started = "Threads", questions = "Questions",links = "URLs", words = "Tokens",tags = "Tags", user_references = "@s",domain_cosine="Cosine(Field)",science_cosine= "Cosine(Science)")

user_count_plot.m <- user_count_plot.m[which(user_count_plot.m$variable %in% c("Comments","Threads","Questions","URLs","Tokens","Tags","@s","Cosine(Field)","Cosine(Science)")),]

commentfeature_grow <- ggplot(user_count_plot.m, aes(user_week, value)) + 
  geom_point(linewidth=.1) + 
  #geom_smooth(size=.7, color="grey") +
...
# difference in people who comment vs. those who do not. Remove testers, those joining befor elaunch
newcomers_data_org$commenter <- ifelse(newcomers_data_org$newcomer_comment==0,0,1)  
newcomers_data_compare <- newcomers_data_org[which(
  !newcomers_data_org$user_id %in% team | 
    !newcomers_data_org$user_id %in% testers | 
    !newcomers_data_org$user_id %in% not_eligible),]


# comparisons of work
wilcox.test(newcomers_data_compare$newcomer_class,newcomers_data_compare$commenter)
data_summary(newcomers_data_compare, varname=c("newcomer_class"), 
                    groupnames=c("commenter"))


wilcox.test(newcomers_data_compare$newcomer_sessions,newcomers_data_org$commenter)
data_summary(newcomers_data_compare, varname=c("newcomer_sessions"), 
                    groupnames=c("commenter"))
```
