...
not_eligible <- newcomers_data$user_id[which(newcomers_data$join_date > max(all_comments$comment_created_at) - months(3))]
...
newcomer_comments$include <- ifelse(newcomer_comments$comment_created_at <= add_with_rollback(newcomer_comments$join_date, months(3), roll_to_first = TRUE),1,0)
...
### BAR 
commentfeature_growB <- ggplot(subset(user_count_plot.summary, variable %in% c("Comments","Questions","URLs","Tokens","Tags","@s")), aes(x=user_week, y=value)) + 
