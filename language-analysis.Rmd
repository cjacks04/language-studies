...
# Issue with system times (they do not overlap in most instances )
```

```{r setup-classifications, include=FALSE, warning=FALSE}
session_builder1 <- data.frame(classifications$classification_id,classifications$user_id,classifications$created_at2,"classification")
names(session_builder1)[c(1:4)] <- c("id","user_id","created_at","type")

session_builder2 <- data.frame(comments$comment_id,comments$comment_user_id,comments$comment_created_at,"comment")
...
explore_82_se <- session_builder[which(session_builder$user_id=="82"),]
...
# Sanity check
explore_82_se <- session_builder[which(session_builder$user_id=="82"),]
explore_82 <- user_session[which(user_session$user_id=="82"),]

## User 33 has 4 hours 5 minutes differnce between submitting comment and next classification

## Go look at a user who commented on a subject and compare the timestamps. 


### Merge dataset about user
#user_history_domain <- merge(user_info,domain_theil, by = "user_id)
#user_history_domain <- merge(user_history_domain,domain_cpd, by = "user_id)
#remove(user_info,domain_theil,domain_cpd)
```
