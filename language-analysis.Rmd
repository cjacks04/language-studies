...
newcomer_info <- session_builder_newcomer %>% 
   ...
   newcomer_weeks = length(unique(floor_date(created_at, "week"))),
   newcomer_days = length(unique(floor_date(created_at, "days"))),
   newcomer_comment_weeks = length(unique(floor_date(created_at, "week")[which(type=="comment")])),

   newcomer_comment_days = length(unique(floor_date(created_at, "days")[which(type=="comment")])),
...
## ## ## ## ## ## ## ## ## ## ## ## ## 
## HOW TO LIMIT WINDOW OF NEWCOMERS? 

newcomer_info_commenters <- newcomer_info[which(newcomer_info$newcomer_weeks >= 2 & newcomer_info$newcomer_comment>=1),]

# Change and actual similarity score
```
...
trainIndex <- createDataPartition(newcomer_info$retained, p = .7,
                                  list = FALSE,
                                  times = 1)
train <- newcomer_info[ trainIndex,]
valid <- newcomer_info[-trainIndex,]

retention.glm <- glm(retained ~ newcomer_class+newcomer_comment+newcomer_sessions,
data = train) # MODEL
logit2prob(coef(retention.glm))
nagelkerke(retention.glm) # performance 

predict <- predict(retention.glm , newdata = valid, type = 'response')
confusionMatrix(data = factor(as.numeric(predict>0.5)), reference = factor(valid$retained))

################################
#   COMMENTERS MODELS          #
################################

trainIndex <- createDataPartition(newcomer_info_commenters$retained, p = .7,
                                  list = FALSE,
                                  times = 1)
train <- newcomer_info_commenters[ trainIndex,]
valid <- newcomer_info_commenters[-trainIndex,]

retention_commenters.glm <- glm(retained ~ newcomer_class+newcomer_comment+newcomer_sessions,
data = train)
logit2prob(coef(retention_commenters.glm))
nagelkerke(retention_commenters.glm) # performance 

predict <- predict(retention_commenters.glm , newdata = valid, type = 'response')
confusionMatrix(data = factor(as.numeric(predict>0.5)), reference = factor(valid$retained))

#NEED: lang.sim + slope  (during newcomer period first four weeks)
#DONE: lurking period + number of tasks + number of comments + sessions
...
```{r}
stargazer(retention.glm,
           omit.stat = "all", digits = 2,type="html")
```
